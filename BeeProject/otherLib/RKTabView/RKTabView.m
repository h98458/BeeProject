...
- (id)initWithFrame:(CGRect)frame andTabItems:(NSArray *)tabItems andSelectionBlock:(selectionBlock)block{
                ...
                switch (tabItem.tabState) {
                    ...
                    case TabStateEnabled:
                        if ([self delegateRespondsToEnableSelector] && notify) {
                            ...
                            //Calling block
                            if(self.selBlock){
                                self.selBlock([self indexOfTab:tabItem]);
                            }
                        }
                        break;
                }
...
- (void)setTabContent:(UIControl *)tab withTabItem:(RKTabItem *)tabItem {
    ...
    if (titleLabel) {
        ...
        titleLabel.frame = CGRectMake(tab.bounds.size.width/2 - titleSize.width/2, wholeGapHeight*2/3+((UIView *)interfaceElement).bounds.size.height-3, titleSize.width+2, titleSize.height+2);
